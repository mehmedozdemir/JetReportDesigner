...
- **A — Report versioning.** A `ReportVersions` table (per-provider migration): a
...
- **B — Filesystem report store.** Extract `IReportStore` (reports + versions);
...
- **C — Designer UX.** Restore marquee selection (regressed in Phase 2); copy /
...
- **D — Validation surfaces.** `ReportDefinitionValidator` also checks that every
...
- **E — Ops & cleanup.**
  - Data Protection keys persisted to `DataProtection:KeyPath` when set
    (`PersistKeysToFileSystem`); compose + Dockerfile mount/þdocument a volume.
  - `samples/reports/*.json` + `samples/data/*.json`; `GET /api/meta/samples` +
    `POST /api/reports/import` so the designer can seed them.
